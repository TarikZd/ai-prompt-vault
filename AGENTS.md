...
- **Formula 2.0 Mastery:** Utilize Formulas 2.0 for all dynamic data (e.g., `ifs()`, `filter()`, `map()`). Create visual progress bars and status indicators using the `slice()` and `repeat()` functions.
- **Button Automations:** Every template must include UX-enhancing buttons.
...
- **SEO Title:** Max 60 characters, keyword-rich (e.g., "Free AI Prompt Vault Notion Template 2026").
...
4. **Alphanumeric Logic:** Every task you create must follow the `[CATEGORY] | [ID] | [IMPACT]` format.
    - *Example:* `[DESIGN] | T1.01 | Dashboard Layout - Impact: /AI Prompt Vault/docs/`
...
## 🧭 Product Directive: AI Prompt Vault

- **Template Name:** AI Prompt Vault
- **SEO Targets:** `ai prompt vault notion`, `chatgpt prompt database`, `claude prompt organizer`, `prompt engineering workspace`, `free prompt manager`
- **Price Point:** Free (lead magnet)
- **Brand:** Moorish Dev | Logo: `assets/moorish_dev_logo.png`
- **Target Audience:** Prompt engineers, content creators, and AI power users looking to organize, test, and version-control prompts.
- **Core Requirement:** Every template must be "Plug and Play." A user should be able to duplicate it and start tracking in under 60 seconds.
